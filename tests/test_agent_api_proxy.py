...
class AgentAPIProxyTest(ServerToolsTestBase):
    ...
    def test_runtime_toml_configures_forwarding_without_env_routing_vars(self):
        ...
        self.write_repo_text(
            ".codebase-tooling-mcp/agent-proxy.local.yaml",
            "agent_proxy:\n"
            "  enabled: true\n"
            "  allow_online: true\n"
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(captured["url"], "https://yaml-provider.example/v1/chat/completions")
        self.assertEqual(payload["agent_proxy"]["routing"]["backend"], "online")
    ...
    def test_agent_proxy_runtime_config_is_gitignored_and_example_is_committed(self):
        ...
        self.assertIn("/.codebase-tooling-mcp/agent-proxy.local.yaml", gitignore_text)
        example = repo_root / "docs/examples/agent-proxy.local.yaml.example"
        self.assertTrue(example.is_file())
        self.assertNotIn("sk-", example.read_text(encoding="utf-8"))
        self.write_repo_text(".gitignore", gitignore_text)
        self.assertTrue(
            self.git("check-ignore", ".codebase-tooling-mcp/agent-proxy.local.yaml").stdout.strip()
        )
    ...
    def test_model_fallback_configure_dry_run_when_mutations_disabled(self):
        ...
        self.assertIn(".codebase-tooling-mcp/agent-proxy.local.yaml", payload["files"])
        self.assertIn("agent_proxy:", payload["files"][".codebase-tooling-mcp/agent-proxy.local.yaml"])
        self.assertIn("Status: dry_run", payload["message"])
        self.assertIn("Secret state:", payload["message"])
        self.assertFalse((self.repo_path / ".continue/model-routing.yaml").exists())
        self.assertFalse((self.repo_path / ".codebase-tooling-mcp/agent-proxy.local.yaml").exists())
    ...
    def test_model_fallback_configure_writes_continue_files_when_allowed(self):
        ...
        agent_proxy_text = (self.repo_path / ".codebase-tooling-mcp/agent-proxy.local.yaml").read_text(
            encoding="utf-8"
        )
    ...
    def test_model_fallback_configure_dry_run_redacts_api_key_and_formats_output(self):
        ...
        self.assertEqual("dry_run", payload["status"])
        self.assertIn("Status: dry_run", payload["message"])
        self.assertIn("Config file: .codebase-tooling-mcp/agent-proxy.local.yaml", payload["message"])
        self.assertIn("Secret state:", payload["message"])
        self.assertIn("agent_proxy:", payload["files"][".codebase-tooling-mcp/agent-proxy.local.yaml"])
        self.assertIn('${{ secrets.AZURE_OPENAI_API_KEY }}', payload["files"][".codebase-tooling-mcp/agent-proxy.local.yaml"])
        self.assertNotIn(raw_key, combined_output)
