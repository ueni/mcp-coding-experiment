...
For agent patch security-regression review, `agent_security_delta` / `agent_security_delta_report` compares changed files between `base_ref` and `head_ref` with local/offline heuristics for newly introduced command injection, path traversal, unsafe deserialization, dynamic code execution, weak temp files, missing HTTP auth checks, and privileged container/configuration changes. It returns redacted repository-relative findings, a pass/warn/block gate, and optional JSON/Markdown/SARIF exports with local provenance sidecars. See [Agent security delta report](./docs/agent-security-delta-report.md).
