...
For agent patch security-regression review, `agent_security_delta` / `agent_security_delta_report` compares changed files between `base_ref` and `head_ref` with local/offline heuristics for newly introduced command injection, path traversal, unsafe deserialization, dynamic code execution, weak temp files, missing HTTP auth checks, and privileged container/configuration changes. It returns redacted repository-relative findings, a pass/warn/block gate, and optional JSON/Markdown/SARIF exports with local provenance sidecars. `security_root_cause_evidence` is a distinct offline/read-only reviewer pack for security-sensitive fixes: it ranks likely root-cause files/functions with reasons and confidence, can reuse compact security-delta findings, surfaces shallow-fix warnings for wrapper/symptom-only or warning-suppression-only changes, feeds a compact `release_readiness` pass/warn/needs-review summary, and explicitly avoids exploit-proof claims from shallow signals. See [Agent security delta report](./docs/agent-security-delta-report.md) and [Security root-cause evidence report](./docs/security-root-cause-evidence.md).
