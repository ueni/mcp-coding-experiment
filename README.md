...
- `.continue/models/*.yaml`, `.continue/model-routing.yaml`, and `.continue/mcpServers/codebase-tooling-mcp.yaml` for the selected local or server-routed OpenAI-compatible Continue profile
...
bundled local Ollama, server-routed OpenAI-compatible backend, or no Continue
model setup. For scripted server-routed setup, pass
`--continue-model-profile openai-compatible` and optional environment overrides.
The backend API base is written to `.codebase-tooling-mcp/agent-proxy.yaml`; the
Continue model profile still points only at `http://localhost:8000/v1`:
...
- The checked-in default Continue Ollama config uses `provider: ollama` with `apiBase: http://127.0.0.1:2345`. The repo also ships `.continue/models/coding-openai-compatible.yaml` as an OpenAI-compatible server-routed profile pointing at `http://localhost:8000/v1`; upstream provider endpoints and API keys belong in the ignored `.codebase-tooling-mcp/agent-proxy.yaml` runtime config, not in Continue or Codex client configs. `.continue/models/model-fallback.yaml` points at `http://localhost:8000/v1/model-fallback` for configuration assistance when no real model is configured yet.
...
- Existing host or repository `.continue` config may stay stale until refreshed. After pulling this change, rebuild/reopen the devcontainer and either keep `MCP_APPLY_REPO_DEFAULTS=true` with `MCP_APPLY_CONTINUE_DEFAULTS=true` or rerun `setup-repository.sh` / manually copy the `source/defaults/continue` files so the host-visible `.continue` config includes the compact default model profile and routing. On an interactive first run, `setup-repository.sh` asks whether Continue should adopt the bundled local Ollama profile, configure a server-routed OpenAI-compatible backend, or skip Continue model setup. For scripted setup, use `--continue-model-profile openai-compatible` plus `CONTINUE_MODEL_ID`, `CONTINUE_MODEL_API_BASE`, and optional `CONTINUE_MODEL_PROXY` / `CONTINUE_MODEL_CA_BUNDLE`; those backend details are stored in the ignored server-side agent-proxy runtime config while Continue routes to `http://localhost:8000/v1`. Use `--continue-model-profile none` when the repository should not receive `.continue` model/MCP profile files, and the generated devcontainer keeps that choice durable with `MCP_APPLY_CONTINUE_DEFAULTS=false`. The fallback assistant chat flow can also write `.continue/models/coding-openai-compatible.yaml`, `.continue/model-routing.yaml`, and `.codebase-tooling-mcp/agent-proxy.yaml` without requiring a direct `/v1/model-fallback/configure` call; when mutations are disabled, it returns a dry-run config payload to mutate-scoped callers.
...
- Schema-backed core tools: `repo_info`, `roots_diagnostics`, `model_assisted_summary`, `runtime_state`, `git_status`, `grep`, `find_paths`, `read_snippet`, `summarize_diff`, `risk_scoring`, `workspace_transaction`, `policy_simulator`, `release_readiness`, `tool_catalog_integrity`, `dependency_security_report`, `ci_workflow_security_report`, `governance_report`, `self_optimization_report`, `artifact_provenance`, `workflow_diagnostics`, `workflow_lineage`, `interaction_invariant_audit`, `mutation_step_guard`
