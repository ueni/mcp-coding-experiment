...
An opt-in proxy can serve `POST /v1/chat/completions` for clients that deliberately configure this server as their OpenAI-compatible `base_url`. It is disabled by default and is not hidden MITM/TLS interception or credential capture. The first slice supports non-streaming responses, `stream: true` SSE chunks, local/offline routing, explicit online provider controls, request-local anonymization, irreversible secret redaction, fail-closed disclosure audit, durable buyer/auditor evidence packets, disclosure summaries, and gated compact memory capture. Routing defaults can come from the local, user-specific `.codebase-tooling-mcp/agent-proxy.local.yaml` generated by the Model Fallback assistant; this runtime file is gitignored and must not contain secrets. A sanitized template is checked in at `docs/examples/agent-proxy.local.yaml.example`. Existing `MCP_AGENT_PROXY_*` environment variables remain supported and override YAML values. See [Explicit Agent API Proxy](./docs/agent-api-proxy.md).
...
- `.continue/models/*.toml`, `.continue/model-routing.toml`, and `.continue/mcpServers/codebase-tooling-mcp.toml` for the selected local or OpenAI-compatible Continue profile
...
- `.continue/models/*.toml` (router + specialist model defaults, repo-owned, skipped when `MCP_APPLY_CONTINUE_DEFAULTS=false`)
- `.continue/model-routing.toml` (routing map for router/specialists, skipped when `MCP_APPLY_CONTINUE_DEFAULTS=false`)
- `.continue/mcpServers/codebase-tooling-mcp.toml` (skipped when `MCP_APPLY_CONTINUE_DEFAULTS=false`)
- `.config/labs/*.json`
...
- The checked-in default Continue Ollama config uses `provider: ollama` with `apiBase: http://127.0.0.1:2345`. The repo also ships `.continue/models/coding-openai-compatible.toml` as an OpenAI-compatible endpoint template for local proxies, hosted endpoints, or MITM inspection setups, plus `.continue/models/model-fallback.toml` pointing at `http://localhost:8000/v1/model-fallback` for configuration assistance when no real model is configured yet.
