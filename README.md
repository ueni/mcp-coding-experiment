...
For GitHub Actions posture review, `ci_workflow_security_report` parses `.github/workflows/*.yml` offline, flags missing/overbroad token permissions, mutable action refs, risky triggers, self-hosted runners, Docker/privileged/publish/secret/artifact signals, supports rationale-and-expiry suppressions, and returns repository-relative redacted evidence. With `export=true`, it writes JSON, Markdown, and SARIF 2.1.0 artifacts with local provenance sidecars; SARIF uses repository-relative locations, stable rule IDs, and deterministic redacted fingerprints, and is never uploaded by the MCP server. See [GitHub Actions workflow security report](./docs/ci-workflow-security.md).
...
For dependency supply-chain review, `dependency_security_report` builds a read-only dependency inventory, matches repository-local or caller-provided advisory data, distinguishes clean/vulnerable/skipped/stale/network-disabled/scanner-unavailable states, and exports JSON, SARIF 2.1.0, plus an optional CycloneDX-compatible SBOM with local provenance sidecars under `.codebase-tooling-mcp/reports/`. It never edits dependency files, uploads SARIF, or performs live network lookup unless a future caller explicitly enables that path; the current offline-safe slice consumes advisory fixtures or externally generated `pip-audit` JSON. See [Dependency security report](./docs/dependency-security.md).
