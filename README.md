...
An opt-in proxy can serve `POST /v1/chat/completions` for clients that deliberately configure this server as their OpenAI-compatible `base_url`. It is disabled by default and is not hidden MITM/TLS interception or credential capture. The first slice supports non-streaming responses, `stream: true` SSE chunks, local/offline routing, explicit online provider controls, request-local anonymization, irreversible secret redaction, fail-closed disclosure audit, durable buyer/auditor evidence packets, disclosure summaries, gated compact memory capture, and an optional tool-response scanner (`MCP_TOOL_RESPONSE_SCANNER_MODE=log|sanitize|block`) before `role: "tool"` messages enter model context. See [Explicit Agent API Proxy](./docs/agent-api-proxy.md) and [MCP tool-response scanner](./docs/tool-response-scanner.md).
...
For GitHub Actions posture review, `ci_workflow_security_report` parses `.github/workflows/*.yml` offline, flags missing/overbroad token permissions, mutable action refs, risky triggers, self-hosted runners, Docker/privileged/publish/secret/artifact signals, supports rationale-and-expiry suppressions, and returns repository-relative redacted evidence with local no-upload SARIF export when `export=true`. See [GitHub Actions workflow security report](./docs/ci-workflow-security.md).

For MCP threat modeling, `mcp_threat_model_report` emits an offline STRIDE/DREAD-style map of MCP host/client, LLM, server, repository, and external-service trust boundaries, links existing controls to threats, returns a frozen deterministic DREAD rubric, analyzes secret-free poisoned-tool fixtures including post-handshake `notifications/tools/list_changed` plus repeated `tools/list` mutations, includes redacted OAuth proxy confused-deputy/token-passthrough hardening outcomes when fixtures or repo-local config are present, and can compare a local baseline for high-severity uncovered regressions. See [MCP threat-model regression report](./docs/mcp-threat-model-report.md).

For dependency supply-chain review, `dependency_security_report` builds a read-only dependency inventory, matches repository-local or caller-provided advisory data, distinguishes clean/vulnerable/skipped/stale/network-disabled/scanner-unavailable states, and exports JSON, local no-upload SARIF, plus an optional CycloneDX-compatible SBOM with local provenance sidecars under `.codebase-tooling-mcp/reports/`. It never edits dependency files or performs live network lookup unless a future caller explicitly enables that path; the current offline-safe slice consumes advisory fixtures or externally generated `pip-audit` JSON. See [Dependency security report](./docs/dependency-security.md).

For secret-exposure review, `secret_exposure_report` scans selected repository paths locally/offline with conservative built-in token, private-key, cloud-key, connection-string, bearer-token, and generic secret-assignment patterns; returns only redacted metadata/fingerprints; supports a repository allowlist; and classifies baseline versus newly introduced findings when Git evidence is available. `mutation_step_guard`, `release_readiness`, and `governance_report` consume the compact redacted summary so new high-confidence in-scope findings can block mutations/releases without exposing raw secrets. See [Secret exposure report](./docs/secret-exposure-report.md).

For agent patch security-regression review, `agent_security_delta` / `agent_security_delta_report` compares changed files between `base_ref` and `head_ref` with local/offline heuristics for newly introduced command injection, path traversal, unsafe deserialization, dynamic code execution, weak temp files, missing HTTP auth checks, and privileged container/configuration changes. It returns redacted repository-relative findings, a pass/warn/block gate, and optional JSON/Markdown/SARIF exports with local provenance sidecars. `security_root_cause_evidence` is a distinct offline/read-only reviewer pack for security-sensitive fixes: it ranks likely root-cause files/functions with reasons and confidence, can reuse compact security-delta findings plus bounded/redacted caller-provided local reproducer/test metadata, surfaces shallow-fix warnings for wrapper/symptom-only or warning-suppression-only changes, feeds a compact `release_readiness` pass/warn/needs-review summary, and explicitly avoids exploit-proof claims from shallow signals. See [Agent security delta report](./docs/agent-security-delta-report.md) and [Security root-cause evidence report](./docs/security-root-cause-evidence.md).

For enterprise audit/release review, `skill_privilege_scope` can feed advisory blockers from imported-card/skill least-privilege review into governance, `workflow_policy_plan` can preflight an intent-declared MCP tool sequence before execution, `policy_governance_decision` can evaluate the same declared steps against a reviewed repo-local `mcp_governance_policy_bundle.v1` without dynamic code loading or bypassing existing hard gates, and `governance_report` reads redacted events from `MCP_AUDIT_LOG_FILE`, summarizes local policy/readiness/tool-chain/snapshot/dependency-security/tool-catalog-integrity/CI-workflow-security/secret-exposure/agent-security-delta/workflow-policy/policy-as-code evidence, and exports JSON plus Markdown plus local provenance sidecars under `.codebase-tooling-mcp/reports/`. It also writes a redacted deterministic `workflow_lineage.v1` manifest for the governance-report workflow; `workflow_lineage(mode="verify")` checks the manifest read-only and reports `matched`, `input_changed`, `artifact_changed`, and `non_deterministic_node` conditions without persisting prompts, transcript snippets, secrets, file contents, or host absolute paths. `artifact_provenance` verifies report sidecars and snapshot-index sidecars read-only, reports existing unsigned sidecars as `unsigned` / `local-only`, validates deterministic offline `local-dsse-fixture` attestations without network calls, and can opt into `github-artifact-attestations` verification when the sidecar/config provides a local bundle, trusted root, expected repository/workflow/ref-or-commit policy, and predicate type. GitHub attestation verification is offline by default; online `gh attestation verify` is unavailable unless explicitly enabled and reports `network_access=true` only when it actually uses network access. Other future backends such as Sigstore/cosign remain `unsupported`. `workflow_task` can start the governance report asynchronously and persist a redacted MCP Tasks-style status handle under `.codebase-tooling-mcp/tasks/`; poll it with `task_status`. `result_reference_resolve` resolves opt-in repository-local `mcp_result_reference.v1` handles for large read-only outputs after boundary, expiry, and SHA-256 checks. `workflow_diagnostics` turns failed audit events and optional caller-supplied trajectory snippets into a redacted critical-step/failure-category report with safe recovery actions. `workflow_event_log_report` projects a local redacted JSONL workflow event log into typed checkpoints, a deterministic timeline/artifact lineage view, and fork-vs-parent diffs without transport replay or tool reruns. `tool_catalog_integrity` compares live public MCP tool, prompt, resource/template, and discovery metadata against the checked-in digest baseline and returns surface-separated added/removed/changed metadata diffs plus advisory metadata-lint findings without exposing repository contents, resource payloads, raw dynamic prompt arguments, or secrets. `mcp_threat_model_report` layers those signals into an offline STRIDE/DREAD fixture regression report for MCP tool-poisoning and client-transparency risks. `self_optimization_report` builds an offline-by-default, repo-local efficiency report for recent MCP usage, token/time savings, cache/compression effects, issue/PR/workflow/task throughput, state transitions, test gates, retries/rework, blocked/waiting time, bottlenecks, confidence/caveats for unknown inputs, duplicate-suppressed optimization candidates, and an advisory optimization-integrity report that checks suspected proxy wins before high-confidence GitHub issue update gating. #88 proxy/anonymizer/disclosure/router data is treated only as non-blocking redacted enrichment for routing/disclosure metrics when available. `interaction_invariant_audit` checks extracted task constraints and interaction-smell risks before mutation or readiness summaries without persisting conversation snippets by default. `trajectory_trust_guard` scores redacted tool-call trajectories for over-trust in untrusted/inconsistent feedback before sensitive final actions without persisting raw prompts or tool outputs. `workflow_reminder` emits compact read-only `workflow_reminder.v1` risk-reminder packets with redacted emission/suppression evidence before high-risk planned actions, reusing existing gate vocabulary rather than granting permission. `grep`, `governance_report`, and `self_optimization_report` can also opt into deterministic `compressed_observation` summaries or reference handles without replacing raw results or artifacts. See [Dependency security report](./docs/dependency-security.md), [GitHub Actions workflow security report](./docs/ci-workflow-security.md), [Agent security delta report](./docs/agent-security-delta-report.md), [Governance report workflow](./docs/governance-report.md), [MCP threat-model regression report](./docs/mcp-threat-model-report.md), [Tool catalog integrity baseline](./docs/tool-catalog-integrity.md), [Self-optimization efficiency report](./docs/self-optimization-report.md), [Reference-based result handles](./docs/result-references.md), [Workflow lineage manifests](./docs/workflow-lineage.md), [Workflow policy plan preflight](./docs/workflow-policy-plan.md), [Policy-as-code governance decision adapter](./docs/policy-governance-decision.md), [Workflow reminder packets](./docs/workflow-reminder.md), [Async workflow tasks](./docs/workflow-tasks.md), [Workflow diagnostics](./docs/workflow-diagnostics.md), [Workflow event-log checkpoint report](./docs/workflow-event-log-report.md), [Interaction invariant audit](./docs/interaction-invariant-audit.md), [Trajectory trust guard](./docs/trajectory-trust-guard.md), and [Adaptive observation compression](./docs/observation-compression.md).
...
- `tool_catalog_integrity` for checked-in public MCP tool/prompt/resource/discovery digest drift checks
...
- Schema-backed core tools: `repo_info`, `roots_diagnostics`, `model_assisted_summary`, `runtime_state`, `git_status`, `grep`, `find_paths`, `read_snippet`, `summarize_diff`, `risk_scoring`, `workspace_transaction`, `policy_simulator`, `workflow_policy_plan`, `skill_privilege_scope`, `policy_governance_decision`, `workflow_reminder`, `release_readiness`, `agent_quality_delta`, `tool_catalog_integrity`, `dependency_security_report`, `ci_workflow_security_report`, `secret_exposure_report`, `agent_security_delta`, `agent_security_delta_report`, `security_root_cause_evidence`, `mcp_threat_model_report`, `agent_lifespan_drift_report`, `governance_report`, `memory_governance_report`, `self_optimization_report`, `workflow_phase_telemetry`, `observation_compression_report`, `agents_context_health`, `artifact_provenance`, `result_reference_resolve`, `workflow_diagnostics`, `workflow_event_log_report`, `workflow_lineage`, `interaction_invariant_audit`, `mutation_step_guard`, `trajectory_trust_guard`

`task_router()` remains the default public entrypoint and now defaults to `mode="task"`. It classifies the request, encodes the routing packet, reads and writes compact task/session memory automatically, and dispatches to the selected specialist flow. Use `mode="workflow_select"` plus `execution_mode="online" | "offline" | "auto"` as a read-only preflight when an agent is unsure which workflow/prompt/tool to choose, and `mode="skill_privilege_scope"` when imported cards/skills need task-conditioned privilege linting. Use `memory_session` when you want related requests to share that compact context or to isolate a separate task thread.
...
`workflow_event_log_report()` is a read-only checkpoint projection for local JSONL workflow event logs. It emits `workflow_checkpoint_report.v1`, reconstructs timeline/artifact lineage from fixtures without rerunning tools, redacts prompts/tool outputs/secrets/host paths, and supports local fork-vs-parent diff reports. See [Workflow event-log checkpoint report](./docs/workflow-event-log-report.md).

`workflow_phase_telemetry()` provides read-only phase-level optimization metrics for redacted workflow/tool-call summaries, including phase counts, cache and repeated-read signals, guard/test/release markers, ordering anomalies, and local-only privacy metadata. See [Workflow phase telemetry](./docs/workflow-phase-telemetry.md).

`self_optimization_report()` is the direct software-team efficiency report for this repository. Run `self_optimization_report(window_hours=168, export=true)` after issue/PR batches or noisy MCP runs to summarize local usage, savings, task/state/test-gate/retry/blocked-time metrics, throughput, bottlenecks, confidence/caveats, duplicate-suppressed optimization candidates, and an advisory `optimization_integrity_report` that checks suspected proxy wins against release/test-gate failures, skips, stale artifacts, retries/rework, suppressions, missing denominators, and review/patch-survivorship pushback without exposing raw traces or secrets. GitHub issue create/update remains off unless explicitly gated with high-confidence candidates and a trusted anti-gaming verdict. See [Self-optimization efficiency report](./docs/self-optimization-report.md). Use `observation_compression_report(window_hours=168, export=true)` for a standalone read-only TACO-style bucket report over stored workflow/task/tool observations; see [Observation compression report](./docs/observation-compression-report.md). The offline E2E MCP workflow benchmark runner writes a sibling summary report that can feed the same optimization loop; see [E2E MCP workflow benchmarks](./docs/e2e-mcp-workflow-benchmarks.md).

`scripts/review_outcome_report.py` consumes redacted local review-comment metadata fixtures and classifies whether automated review findings were `resolved_by_change`, `accepted_no_change`, `dismissed`, `stale`, `open`, `unverifiable`, or `regressed` after follow-up commits. It is offline/read-only by default, reports aggregate resolution/noise/evidence metrics and gate-state correlations, and does not store raw comments or full patches. See [Review outcome report](./docs/review-outcome-report.md).

`agents_context_health()` is a read-only minimal-context lint report for `AGENTS.md`. It emits byte/estimated-token budget status, duplicate and stale guidance candidates, risky global instruction signals, instruction categories, and move-to-router/docs candidates without returning raw AGENTS text, writing artifacts, or using network access. See [AGENTS.md context health report](./docs/agents-context-health.md).
...
`tool_annotations()` returns machine-checkable read-only/destructive/idempotent/open-world hints for the public tools and covered public modes such as `task_router`, `test_impact_map(refresh=true)`, `workflow_task(start)`, and `workspace_transaction`. The schema-backed core tools publish checked-in output contracts for clients that validate `structuredContent`; `tool_output_contracts()` returns those contracts and the shared error envelope. `agent_quality_delta()` adds a read-only base/head maintainability delta gate for autonomous PRs and is summarized by `release_readiness`; see [Agent quality delta](./docs/agent-quality-delta.md). `agent_lifespan_drift_report()` runs deterministic offline longitudinal fixtures for memory/workflow-card revision, interference, compression, and utilization drift and is summarized by `governance_report`; see [Agent lifespan drift report](./docs/agent-lifespan-drift-report.md). `tool_catalog_integrity()` hashes the public `mcp.list_tools()`, prompt catalog/template, resource/template, allowlisted discovery/docs metadata, annotations, output contracts, categories, and documentation references against `source/tool_catalog_baseline.json`; see [Tool catalog integrity baseline](./docs/tool-catalog-integrity.md). `scripts/tool_contract_fuzzer.py` complements that static baseline with dynamic, deterministic read-only calls that validate runtime outputs, error envelopes, and redaction invariants; see [MCP tool contract behavioral fuzzing](./docs/tool-contract-fuzzing.md). Leaf implementations remain in `source/server.py` as direct call targets for router orchestration and for internal tests.

`workflow_reminder()` emits deterministic read-only `workflow_reminder.v1` reminder packets before high-risk planned actions such as missing rollback before mutation, stale tests before readiness, secret-sensitive actions, scope expansion, or repeated failed mutation attempts. It returns remembered constraints, the next existing gate to run, safe next actions, and redacted emission/suppression evidence without executing tools or granting permission. See [Workflow reminder packets](./docs/workflow-reminder.md).

`mutation_step_guard()` is a deterministic read-only final checkpoint before write/delete/git/workspace-transaction operations. It compares the planned tool/mode, argument summary, declared intent, target files, expected diff shape, rollback/snapshot evidence, selected tests or impact-gate status, invariant-audit summary, and freshness metadata, then returns `allow`, a concrete missing-precondition decision, `needs_human_approval`, or `deny` without executing the mutation. See [Mutation step guard](./docs/mutation-step-guard.md).

`trajectory_trust_guard()` is a deterministic read-only advisory guard for sensitive final actions that depend on tool-call trajectories. It scores source diversity, single-tool dependency, scanner-warning accumulation, inconsistent feedback, untrusted high-confidence dependency, provenance gaps, and final-action sensitivity; returns `pass`, `warn`, or `block`; and links optional integration points for `mutation_step_guard`, `release_readiness`, and `governance_report` without changing default enforcement. See [Trajectory trust guard](./docs/trajectory-trust-guard.md).
