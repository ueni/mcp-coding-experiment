...
| all scenarios | current-orchestrator | n/a (non-streaming) | 0.031 median | estimated | estimated | process RSS delta max 0.75 MB; no GPU | degraded comparison | `7/7` task_router harness invocations; `tool_fallback`/unavailable local inference path, token counts approximate |
