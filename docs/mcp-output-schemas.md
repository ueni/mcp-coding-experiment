...
| `governance_report` | `schema`, `report_id`, `generated_at`, `audit`, `governance_hooks`, `agents_context_health`, `exports`, `resource_links` | `window`, `git`, `snapshots`, `security`, `workflow_diagnostics`, `tool_catalog_integrity`, `ci_workflow_security`, `secret_exposure`, `agent_security_delta`, aggregate `untrusted_content_signals`, compact `memory_governance`, `result_references`, `result_mode`, `result_reference`, `lineage`, `provenance`, opt-in `compressed_observation`, `_meta` |
