...
| `skill_privilege_scope` | `schema`, `read_only`, `executed_imported_code`, `external_services_called`, `summary`, `items`, `action_nodes`, `required_privileges`, `advisory_blockers`, `governance` | redacted task intent profile, repository-relative action nodes, declared privilege details, advisory blockers, and security metadata |
