...
| `mcp_threat_model_report` | `schema`, `report_id`, `generated_at`, `status`, `ok`, `summary`, `components`, `trust_boundaries`, `threats`, `findings`, `exports` | controls, fixture/baseline metadata, resource links, and offline STRIDE/DREAD evidence |
