...
| `security_root_cause_evidence` | `schema`, `report_id`, `generated_at`, `base_ref`, `head_ref`, `status`, `ok`, `summary`, `ranked_locations`, `evidence_inputs`, `security` | read-only/advisory flags, changed files, related tests, import neighbors, shallow-fix warnings, limitations, and redacted reasons/confidence |
