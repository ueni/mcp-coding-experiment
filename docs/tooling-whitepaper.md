...
`mcp_threat_model_report` is the offline public governance tool for MCP trust-boundary review. It maps host/client, LLM, server, repository, and external-service boundaries to STRIDE/DREAD-style threats, links existing controls to risks, analyzes secret-free poisoned-tool fixtures, and can compare a local baseline for high-severity uncovered regressions without exposing secrets or raw external state.
