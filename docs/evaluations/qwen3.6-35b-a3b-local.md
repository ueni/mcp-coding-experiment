...
Status: GPU-backed target-runtime evaluation record for `ueni/mcp-coding-experiment#1` with checked-in current-orchestrator comparison evidence. The revised issue #1 evaluation artifacts satisfy the reproducibility, scenario coverage, measurement, GitHub-hosted artifact-validation, limitations, and final-recommendation criteria for a **limited/offline** viability recommendation. The remaining limitation is comparison fidelity: the current-orchestrator harness measures the repository task router's non-streaming, degraded `tool_fallback` path available in this environment, not a hosted/default-assistant production endpoint. The selected public GGUF is present/checksummed locally, Docker/Ollama can load it with Vulkan/RADV, and a full seven-scenario bounded target-model run completed on 2026-05-09 with `offloaded 41/41 layers to GPU`. The measured median `8.056` sustained tokens/sec meets the revised approximately 7 sustained tokens/sec throughput threshold.
