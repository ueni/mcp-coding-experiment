...
Record the exact backend used to obtain and serve the model. The recommended local route is an OpenAI-compatible local inference endpoint so the same harness can call both local model and orchestrator-like interfaces.
