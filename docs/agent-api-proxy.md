...
The same forwarding defaults can be managed through a local, user-specific YAML file generated by the Continue Model Fallback assistant:

```text
.codebase-tooling-mcp/agent-proxy.local.toml
```

The runtime TOML file is covered by `.gitignore` and must not be committed. Commit only the sanitized example at `docs/examples/agent-proxy.local.toml.example` if a reference is needed. Keep provider API keys and bearer tokens in environment variables or Continue secrets; the YAML uses provider-style fields (`provider`, `model`, `apiBase`, `apiType`, `apiVersion`, `apiKey`) and stores only a Continue secret reference/symbolic name for `apiKey`. If a real provider such as Azure/OpenAI needs a key and no usable secret exists, the setup assistant asks for it through the mutation-gated configure flow and stores the raw value only in Continue secrets. Existing `MCP_AGENT_PROXY_*` environment variables remain supported and override TOML values.
