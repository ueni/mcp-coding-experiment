...
The runtime YAML file is covered by `.gitignore` and must not be committed. Commit only the sanitized example at `docs/examples/agent-proxy.local.yaml.example` if a reference is needed. Keep provider API keys and bearer tokens in environment variables or Continue secrets; the YAML uses provider-style fields (`provider`, `model`, `apiBase`, `apiType`, `apiVersion`, `apiKey`) and stores only a Continue secret reference/symbolic name for `apiKey`. Existing `MCP_AGENT_PROXY_*` environment variables remain supported and override YAML values.
