...
When `MCP_APPLY_REPO_DEFAULTS=true`, startup refreshes the repository-managed Continue defaults, including the Qwen3.6 profile, the `Coding Agent - Llama 3.1 8B` profile, and routing entries. This preserves custom unrelated model files while repairing stale repo defaults copied before the context/tool-capability fixes. Existing host or repository `.continue` config may remain stale until refreshed; after pulling these defaults, rebuild/reopen the devcontainer and keep `MCP_APPLY_REPO_DEFAULTS=true`, rerun `setup-repository.sh`, or manually copy `source/defaults/continue` into `.continue` so the host-visible Continue config picks up the Llama agent profile.
